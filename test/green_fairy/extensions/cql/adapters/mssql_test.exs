defmodule GreenFairy.CQL.Adapters.MSSQLTest do
  ...
  describe "with bindings" do
    test "applies operator with binding" do
      ...
      result = MSSQL.apply_operator(query, :name, :_eq, "John", binding: :user, field_type: :string)
      assert Helper.has_where?(result)
    end

    test "applies OPENJSON operator with binding" do
      query = from(u in Helper.User, as: :post)
      result = MSSQL.apply_operator(query, :tags, :_includes, "premium", binding: :post, field_type: {:array, :string})
      assert Helper.has_where?(result)
      assert Helper.has_fragment?(result, "OPENJSON")
    end
  end
