defmodule GreenFairy.CQL.Scalars.Integer.Ecto do
  @moduledoc "Ecto/SQL implementation for integer operators"
  ...
  defp apply_gt(query, field, value, nil), do: where(query, [q], field(q, ^field) > ^value)

  defp apply_gt(query, field, value, binding),
    do: where(query, [{^binding, a}], field(a, ^field) > ^value)

  defp apply_gte(query, field, value, nil), do: where(query, [q], field(q, ^field) >= ^value)

  defp apply_gte(query, field, value, binding),
    do: where(query, [{^binding, a}], field(a, ^field) >= ^value)

  defp apply_lt(query, field, value, nil), do: where(query, [q], field(q, ^field) < ^value)

  defp apply_lt(query, field, value, binding),
    do: where(query, [{^binding, a}], field(a, ^field) < ^value)

  defp apply_lte(query, field, value, nil), do: where(query, [q], field(q, ^field) <= ^value)

  defp apply_lte(query, field, value, binding),
    do: where(query, [{^binding, a}], field(a, ^field) <= ^value)
